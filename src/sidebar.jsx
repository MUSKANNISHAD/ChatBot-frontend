...
function Sidebar({ showSidebar, setShowSidebar }) {
  ...
  const getAllThreads = async () => {
    try {
      const response = await fetch("https://chatbot-backend-tm7h.onrender.com/threads");
  ...
  const changeThread = async (newThreadId) => {
    ...
    try {
      const response = await fetch(`https://chatbot-backend-tm7h.onrender.com/thread/${newThreadId}`);
      const data = await response.json();
  ...
  const deleteThread = async (threadId) => {
    try {
      const response = await fetch(`https://chatbot-backend-tm7h.onrender.com/thread/${threadId}`, {
        method: "DELETE"
      });

...
function Sidebar({ showSidebar, setShowSidebar }) {
  ...
  const getAllThreads = async () => {
    try {
      const response = await fetch("http://ec2-56-228-49-72.eu-north-1.compute.amazonaws.com:5000/threads");
  ...
  const changeThread = async (newThreadId) => {
    ...
    try {
      const response = await fetch(`http://ec2-56-228-49-72.eu-north-1.compute.amazonaws.com:5000/thread/${newThreadId}`);
      const data = await response.json();
  ...
  const deleteThread = async (threadId) => {
    try {
      const response = await fetch(`http://ec2-56-228-49-72.eu-north-1.compute.amazonaws.com:5000/thread/${threadId}`, {
        method: "DELETE"
      });

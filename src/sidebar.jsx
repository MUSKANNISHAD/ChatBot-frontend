...
function Sidebar({ showSidebar, setShowSidebar }) {
  ...
  const getAllThreads = async () => {
    try {
      const response = await fetch("https://stayhub-chat.duckdns.org:5000/threads");
  ...
  const changeThread = async (newThreadId) => {
    ...
    try {
      const response = await fetch(`https://stayhub-chat.duckdns.org:5000/thread/${newThreadId}`);
      const data = await response.json();
  ...
  const deleteThread = async (threadId) => {
    try {
      const response = await fetch(`https://stayhub-chat.duckdns.org:5000/thread/${threadId}`, {
        method: "DELETE"
      });
